...
export function EditClaimForm({ claim, policies }: Props) {
  const router = useRouter();
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [success, setSuccess] = useState(false);

  const [claimNumber, setClaimNumber] = useState(String(claim.claim_number ?? ""));
  const [policyId, setPolicyId] = useState(String(claim.policy_id ?? ""));
  const [status, setStatus] = useState(String(claim.status ?? "reported"));
  const [lossDate, setLossDate] = useState(toDateStr(claim.loss_date));
  const [reserveAmount, setReserveAmount] = useState(String(claim.reserve_amount ?? "0"));
  const [paidAmount, setPaidAmount] = useState(String(claim.paid_amount ?? "0"));
  ...
  if (success) {
    return (
      <div className="mt-8 rounded-xl border border-emerald-200 bg-emerald-50 p-6">
        <p className="font-semibold text-emerald-800">Claim updated</p>
